...
export default {
  ...
  methods: {
    ...
    initSocket() {
      ...
      this.socket.emit('stepresponse');
      this.socket.on('step_connect', (data) => {
        this.$message.success("用户步数更新")
    ...
    handleSocketData(data) {
      console.log('Received data1111:', data);
      if (data.error) {
        console.error(data.error);
      } else {
        // const heartRateArray = Object.values(data['Heart Rate']);
        const stepsArray = Object.values(data['Step']);
        const timeArray = Object.values(data['Formatted Date']);


        this.heartRateLabels = timeArray;
