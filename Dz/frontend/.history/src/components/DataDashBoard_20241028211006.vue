  ...
  <div class="datadashboard">
    ...
    <div class="sidebar">
      <ul>
        ...
        <!-- 仅当用户为医生时显示医生访问入口 -->
        <li v-if="isDoctor" @click="selectModule('DoctorAccess')">医生访问</li>
      </ul>
...
import DoctorAccess from './dashboard/DoctorAccess.vue';
...
export default {
  ...
  computed: {
    ...
    isDoctor() {
      return this.userRole === 'doctor'; // 判断用户是否为医生
    }
  },
  methods: {
    selectModule(module) {
    ...
    handleLoginSuccess(userRole) {
      this.userRole = userRole; // 在登录成功后设置用户身份
      if (userRole === 'doctor') {
        this.activeModule = 'DoctorAccess';
      }
    },
