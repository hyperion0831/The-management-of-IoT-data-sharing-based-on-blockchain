<template>
  ...
  export default {
    ...
    mounted() {
      console.log(this.$router); // 确保 router 存在
      // 页面加载时检查是否已登录
      ...
      async verifyKeyPair(publicKeyBase64, privateKeyBase64) {
        ...
        try {
          ...
          if (matchingKey) {
            ...
            // 页面跳转到 DataDashboard
            console.log(`当前路径：${this.$router.currentRoute.value.path}`);
            console.log(`当前路由名称：${this.$router.currentRoute.value.name}`);

            this.$router.push({ name: 'DataDashboard' }).catch((err) => {
              console.error('跳转失败:', err);
            });
            console.log(`router路径：${this.$router}`); // 确保 this.$router 存在
            
            // 使用 return 结束函数，避免后续的 catch 块执行
            return;
          } else {
            // 未找到匹配的密钥对时弹窗提示
