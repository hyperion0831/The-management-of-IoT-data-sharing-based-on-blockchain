<template>
  <div class="login-wrapper">
    <!-- 登录/注册界面标题 -->
    <h1 v-if="!isRegistering">欢迎使用智能手表健康监测系统</h1>

    <!-- 登录、注册按钮 -->
    <div v-if="!isRegistering && !isLoggedIn" class="button-row">
      <button @click="toggleView('register')">注册</button>
      <button @click="toggleView('login')">登录</button>
    </div>

    <!-- 登录界面 -->
    <div v-if="currentView === 'login' && !isRegistering && !isLoggedIn" class="login-section">
      <h2>登录</h2>
    ...
    <!-- 登录成功信息显示 -->
    <div v-if="isLoggedIn && !isRegistering" class="blockchain-info">
      <h2>登录成功</h2>
      <label>区块链用户名：</label>
