  ...
  methods: {
    ...
    handleFileUpload(event) {
      ...
      if (file) {
        ...
        reader.onload = () => {
          ...
          console.log(`handle的私钥：${privateKeyMatch}`);
    ...
    async verifyKeyPair(publicKeyBase64, privateKeyBase64) {
      try {
        ...
        console.log(`私钥：${privateKeyBase64}`);
        for (let i = 0; i < localStorage.length; i++) {
          const key = localStorage.key(i);//user-TYC-
          const storedData = JSON.parse(localStorage.getItem(key));
          console.log(`解析的私钥：${storedData.privateKey}`);
        ...
        if (matchingKey) {
          ...
          sessionStorage.setItem("userName", matchingKey.split('-')[1]); // 假设姓名在key的第二段
          alert(`登录成功！\n欢迎你， ${matchingKey.split('-')[1]}\n${matchingKey.startsWith('doctor') ? '医生' : '用户'}`);
        } else {
