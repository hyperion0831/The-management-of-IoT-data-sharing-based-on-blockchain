  ...
  methods: {
    ...
    async verifyKeyPair(fileContent) {
      try {
          ...
          const privateKeyBase64 = privateKeyMatch[1].trim();

          // 生成 registrationKey 格式为 'user-DDS-'
          const registrationKey = `user-${this.name}-`;
          console.log("登录时生成的 registrationKey:", registrationKey);

          // 检查是否存在对应的注册信息
          const registeredData = localStorage.getItem(registrationKey);
          if (!registeredData) {
              alert("未找到注册信息，请检查密钥文件。");
              return;
          }
    ...
    handleFileUpload(event) {
      ...
      if (file) {
        ...
        reader.onload = (event) => {
          ...
          console.log("当前用户的姓名233:", this.name);
