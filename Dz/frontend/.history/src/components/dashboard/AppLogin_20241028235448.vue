...
export default {
  ...
  mounted() {
    // 页面加载时检查是否已登录
    const isLoggedIn = sessionStorage.getItem("${registrationKey}-isLoggedIn");
    const userName = sessionStorage.getItem("${registrationKey}-userName"); // 从 sessionStorage 获取用户名称
  ...
  methods: {
    ...
    async verifyKeyPair(fileContent) {
        try {
          ...
          // 解密成功，密钥匹配，设置登录状态
          this.isLoggedIn = true;
          // 成功后，将用户状态按唯一键存入 sessionStorage
          sessionStorage.setItem(`${registrationKey}-isLoggedIn`, "true");
          sessionStorage.setItem(`${registrationKey}-userName`, this.name);
