...
export default {
  ...
  methods: {
      ...
      try {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            storedData.privateKey === privateKeyBase64) 
          {
            ...
            console.log(`键值：${this.matchingKey}`);
            break;
          }
        }
        if (this.matchingKey) {
          console.log(`找到匹配的 registrationKey：${this.matchingKey}`);
          // 执行登录逻辑，例如设置登录状态、保存 session 等
          this.isLoggedIn = true;
          
          sessionStorage.setItem("isLoggedIn", "true");
          sessionStorage.setItem("userName", this.matchingKey.split('-')[1]); // 假设姓名在key的第二段
          alert(`登录成功！\n欢迎你， ${this.matchingKey.split('-')[1]}${this.matchingKey.startsWith('doctor') ? '医生' : '用户'}`);
        } else {
    ...
    clearRegistration() {
      ...
      console.log("尝试清除的注册信息键：", this.matchingKey);
      if (localStorage.getItem(this.matchingKey)) {
          // 删除该键
          localStorage.removeItem(this.matchingKey);
